# # #torch, vision and audio has been installed, now start installing geometric and its optional dependencies - 39th Time
# # #Investigate about Meta Layer and confirm its modular code
# # #
#
# import sys, os
# import glob
...
#   fns = sorted(list(glob.glob('data/gdb9_fragment_embedded_graphs/gdb9_skeleton_graphs_nocg_1a_attr300.npy')))
#
#   if load_split:
...
##Above code is ERIC's and working fine
...
def load_targets(targ_fn, which_idx=-1):  # No changes here
    if type(targ_fn) != list:
        targ_fn = [targ_fn]
    targ_dict = dict()
    for targ_fn_ in targ_fn:
...
if __name__ == "__main__":
    ...
    # **Changed**: Updated the model initialization to use edge-wise predictions
    if load_pretrained_model:
        model = load_model(model_path)
    else:
        model = FragGraph(X_tr_batches[0])

    # train model
    if mode == 1:
        model = train(model, X_tr_batches, num_iters=10, load_model=False, save_model=False)
