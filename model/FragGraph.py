...
#                                     Linear(dim3, 1)
#                                  ))
#         # self.decode_edges = torch.nn.ModuleList()
#         # self.decode_edges.append(Sequential(
#         #     Linear(self.latent_size['edges'], dim1), ReLU(),
#         #     Linear(dim1, dim2), ReLU(),
#         #     Linear(dim2, dim3), ReLU(),
#         #     Linear(dim3, 1)
#         # ))
...
#         # e_out = self.decode_edges[0](e_out)
#
#
#         out = torch_geometric.nn.global_add_pool(n_out, data.batch)
#         # out = scatter_add(e_out, data.edge_index[0], dim=0)
#         # out = global_add_pool(out, data.batch)
#
#         return out, n_out
        # return out, e_out

### Above code is working and it is making predictions from Node and Edge both - ERIC's code
...
class EdgeModel(torch.nn.Module):
    ...
    def forward(self, src, dest, edges, globals, batch):
        comb = torch.cat([src, dest, edges], dim=1)

        print(f"src shape: {src.shape}")
        print(f"dest shape: {dest.shape}")
        print(f"edges shape: {edges.shape}")
        print(f"Combined shape (comb): {comb.shape}")
...
class FragGraph(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        e_out = self.decode_edges[0](e_out)  # Decode edge features

        # Sum edge features for graph-level prediction
        out = torch_geometric.nn.global_add_pool(e_out, data.batch)

        return out, e_out  # Return edge-level predictions as well
